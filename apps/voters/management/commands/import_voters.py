...
class Command(BaseCommand):
    help = 'Import voter data from Excel files in nested folder structure'
    ...
    def _process_excel_files(self, excel_files, category):
        """Process Excel files and import voter data"""
        for excel_path in excel_files:
            ...
            try:
                ...
                # Get existing voter_no values to avoid duplicates
                existing_voter_nos = set(Voter.objects.values_list('voter_no', flat=True))
                ...
                for _, row in df.iterrows():
                    ...
                    # Skip if voter_no already exists
                    if voter_no and voter_no in existing_voter_nos:
                        skipped_count += 1
                        continue
                    
                    # Add to existing set to prevent duplicates within same file
                    if voter_no:
                        existing_voter_nos.add(voter_no)
                ...
                msg = f'    Imported {len(voters_to_create)} voters from {excel_path.name}'
                if skipped_count > 0:
